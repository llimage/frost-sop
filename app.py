...
_IS_TESTING = (
    ...
    or os.environ.get("FROST_TESTING") == "1"
)
    ...
    with ui.tab_panels(tabs, value=tab_home).classes("w-full bg-gray-900"):
        ...
        with ui.tab_panel(tab_home):
            ...
            with ui.row().classes("w-full gap-2 mt-2"):
                ui.button(
                    "🆕 新功能开发",
                    on_click=lambda: (
                        task_input.set_value("给项目添加用户认证功能"),
                        sop_select.set_value("DEV-001"),
                    ),
                ).classes("flex-1")
                ui.button(
                    "🐛 Bug修复",
                    on_click=lambda: (
                        task_input.set_value("修复登录页面的样式错乱问题"),
                        sop_select.set_value("DEV-002"),
                    ),
                ).classes("flex-1")
                ui.button(
                    "📋 项目立项",
                    on_click=lambda: (
                        task_input.set_value("调研AI Agent记忆系统并立项"),
                        sop_select.set_value("STR-001"),
                    ),
                ).classes("flex-1")
                ui.button(
                    "📝 内容发布",
                    on_click=lambda: (
                        task_input.set_value("撰写本周技术周报"),
                        sop_select.set_value("MT-001"),
                    ),
                ).classes("flex-1")
            ...
            home_log = (
                ...
                .classes("w-full h-64 text-green-400")
                .style(
                    ...
                    with kpi_row:
                        ...
                        with ui.card().classes("flex-1"):
                            with ui.card_section():
                                ...
                                ui.label(
                                    f"完成率 {completed * 100 // max(total, 1)}%"
                                ).classes("text-caption text-green-500")
                        with ui.card().classes("flex-1"):
                            with ui.card_section():
                    ...
                    with kpi_row:
                        with ui.card().classes("flex-1"):
                            with ui.card_section():
                                ...
                                ui.label("请启动 FastAPI").classes(
                                    "text-caption text-yellow-500"
                                )
        ...
        with ui.tab_panel(tab_arsenal):
            ...
            for sop_id, sop_name in SOP_TEMPLATES.items():
                with ui.expansion(f"{sop_id} - {sop_name}", icon="description").classes(
                    "w-full mb-2"
                ):
        ...
        with ui.tab_panel(tab_health):
            ...
            ui.button(
                "🔍 手动触发审计", on_click=lambda: ui.notify("审计已启动，请稍候...")
            ).classes("mt-4")
            ...
            with ui.card().classes("w-full"):
                with ui.card_section():
                    ...
                    ui.label("审计结果：家族健康度良好，未发现重大问题。").classes(
                        "mt-2"
                    )
        ...
        with ui.tab_panel(tab_tasks):
            ...
            refresh_btn = ui.button(
                "🔄 刷新", on_click=lambda: asyncio.create_task(_refresh_tasks())
            )
                            ...
                            with ui.card().classes("w-full"):
                                with ui.card_section().classes(
                                    ...
                                    with ui.column().classes("flex-1 gap-0"):
                                        ...
                                        ui.label(f"ID: {task_id} | {created}").classes(
                                            "text-caption"
                                        )
                                        if summary:
                                            ui.label(summary[:80]).classes(
        ...
        with ui.tab_panel(tab_logs):
            ...
            log = (
                ...
                .classes("w-full h-96 text-green-400")
                .style(
                    "background-color: #0F172A; font-family: monospace; border: 1px solid #334155; border-radius: 6px;"
                )
            )
