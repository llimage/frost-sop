...
try:
    ...
    with (
        ...
        .style(
            "background-color: #0f172a; display: flex; flex-direction: column; padding: 16px; box-sizing: border-box;"
        )
    ):
        # ===== 标题区 =====
        with (
            ...
            ui.label("S-O-P V1.0-MVP 实验原型").classes("text-xl font-bold").style(
                "color: #f8fafc;"
            )
        ...
        with (
            ...
            for title, value, color in kpi_data:
                with (
                    ...
                    .classes("flex-1")
                    .style(
                        "background-color: #1e293b; padding: 16px; border-radius: 8px; border: 1px solid #334155;"
                    )
                ):
        ...
        # ===== 主内容区: 左右分栏 =====
        with (
            ui.row().classes("w-full").style("flex-grow: 1; gap: 16px; min-height: 0;")
        ):
            # === 左侧: 75% ===
            with ui.column().style(
                "width: 75%; gap: 16px; min-height: 0; overflow-y: auto;"
            ):
                # -- Agent网格 (3个卡片横向) --
                with ui.row().classes("w-full gap-4").style("flex-shrink: 0;"):
                    for agent_id, data in agent_status.items():
                        with (
                            ...
                            .classes("flex-1")
                            .style(
                                "background-color: #1e293b; padding: 16px; border-radius: 8px; border: 1px solid #334155; min-width: 0;"
                            )
                        ):
                            # 头部: 图标 + 名称 + 状态灯
                            with (
                                ...
                                ui.label(data["name"]).classes("font-bold").style(
                                    "color: #f8fafc; font-size: 15px;"
                                )
                                ui.element("div").classes("w-3 h-3 rounded-full").style(
                                    f"background-color: {status_colors[data['status']]}; margin-left: auto;"
                                )

                            # 状态文本
                            ui.label(status_texts[data["status"]]).style(
                                f"color: {status_colors[data['status']]}; font-size: 13px; margin-bottom: 12px;"
                            )
                            ...
                            with (
                                ...
                                ui.label("进度:").style(
                                    "color: #64748b; font-size: 12px; width: 36px;"
                                )
                                ui.linear_progress(
                                    value=data["progress"] / 100
                                ).classes("flex-grow").style("height: 6px;")
                                ui.label(f"{data['progress']}%").style(
                                    "color: #64748b; font-size: 12px; width: 32px; text-align: right;"
                                )
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        "background-color: #0f172a; padding: 16px; border-radius: 8px; border: 1px solid #334155; flex-shrink: 0;"
                    )
                ):
                    ui.label("📋 系统日志").classes("font-bold").style(
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        "background-color: #1e293b; padding: 16px; border-radius: 8px; border: 1px solid #334155; flex-shrink: 0;"
                    )
                ):
                    ui.label("📄 任务结果").classes("font-bold").style(
                        "color: #f8fafc; font-size: 15px; margin-bottom: 12px;"
                    )
                    with (
                        ui.column()
                        ...
                        ui.label("暂无执行结果").style(
                            "color: #64748b; font-size: 14px; margin-top: 8px;"
                        )
                        ui.label("在右侧CEO面板输入任务主题").style(
            ...
            with ui.column().style("width: 25%; gap: 16px; min-height: 0;"):
                with (
                    ...
                    .style(
                        "background-color: #1e293b; padding: 20px; border-radius: 8px; border: 1px solid #334155; display: flex; flex-direction: column;"
                    )
                ):
                    # CEO信息
                    with (
                        ...
                        with ui.column():
                            ui.label("老板").classes("font-bold").style(
                                "color: #f8fafc; font-size: 16px;"
                            )
                            ui.label("创始人 / CEO").style(
                                "color: #64748b; font-size: 12px;"
                            )

                    ui.element("div").style(
                        "height: 1px; background-color: #334155; margin-bottom: 20px; flex-shrink: 0;"
                    )

                    # 当前执行计划
                    ui.label("当前执行计划").style(
                        "color: #64748b; font-size: 13px; margin-bottom: 8px; flex-shrink: 0;"
                    )
                    ui.label("界面重构专项\n进行中").style(
                        "color: #f8fafc; font-size: 14px; line-height: 1.6; margin-bottom: 20px; flex-shrink: 0;"
                    )
                    ...
                    with (
                        ...
                        for label, cmd in shortcuts:
                            ui.button(
                                label,
                                on_click=lambda c=cmd: ui.notify(
                                    f"发送: {c}", position="top"
                                ),
...
print("[Exp-1 v2] 请在浏览器打开: http://localhost:8501")
print("[Exp-1 v2] 按 Ctrl+C 停止服务器")
