...
try:
    ...
    with ui.element("div").style(
        ...
        with (
            ...
            .style(
                "padding: 16px 24px; flex-shrink: 0; border-bottom: 1px solid "
                + BORDER
                + ";"
            )
        ):
        ...
        with (
            ...
            with ui.column().style(
                "width: 75%; height: 100%; overflow-y: auto; padding: 20px 24px; gap: 20px; box-sizing: border-box;"
            ):
                # -- 标题区 --
                with ui.column().style("gap: 4px; margin-bottom: 8px;"):
                    ui.label("公司仪表盘").classes("font-bold").style(
                        f"color: {TEXT_PRIMARY}; font-size: 22px;"
                    )
                    ui.label("心域探险项目 - 进行中").style(
                ...
                with ui.row().classes("w-full gap-4").style("margin-bottom: 4px;"):
                    ...
                    for big, small, unit in kpi_items:
                        with (
                            ...
                            .classes("flex-1")
                            .style(
                                f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 10px; padding: 20px;"
                            )
                        ):
                            with ui.row().classes("items-baseline gap-1"):
                                ui.label(big).classes("font-bold").style(
                                    f"color: {TEXT_PRIMARY}; font-size: 32px;"
                                )
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 10px; padding: 16px 20px;"
                    )
                ):
                    with (
                        ui.row()
                        ...
                        with ui.row().classes("items-center gap-3"):
                            ...
                            ui.label("剩余: ¥276.50").style(
                                f"color: {STATUS_RUN}; font-size: 12px;"
                            )
                    # 进度条
                    with (
                        ui.element("div")
                        .classes("w-full")
                        .style(
                            f"height: 8px; background-color: {BG_PAGE}; border-radius: 4px; overflow: hidden;"
                        )
                    ):
                        ui.element("div").style(
                            f"width: 7.8%; height: 100%; background-color: {ACCENT_BLUE}; border-radius: 4px;"
                        )
                    with (
                        ui.row()
                        .classes("w-full items-center justify-between")
                        ...
                        ui.label("80% 预警线").style(
                            f"color: {STATUS_WAIT}; font-size: 12px;"
                        )

                # -- AI Agent 团队 (3个横向卡片) --
                with ui.column().classes("w-full").style("gap: 12px;"):
                    ui.label("AI Agent 团队").classes("font-bold").style(
                        f"color: {TEXT_PRIMARY}; font-size: 16px; margin-bottom: 4px;"
                    )
                    ...
                    with ui.row().classes("w-full gap-4"):
                        for agent in agents:
                            ...
                            with (
                                ...
                                .classes("flex-1")
                                .style(
                                    f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 10px; padding: 16px; position: relative; overflow: hidden;"
                                )
                            ):
                                # 顶部状态条
                                ui.element("div").style(
                                ...
                                with (
                                    ...
                                    with ui.column().style("gap: 2px;"):
                                        ui.label(agent["name"]).classes(
                                            "font-bold"
                                        ).style(
                                            f"color: {TEXT_PRIMARY}; font-size: 15px;"
                                        )
                                ...
                                ui.label(status_texts[agent["status"]]).style(
                                    f"color: {status_color}; font-size: 13px; font-weight: 500; margin-bottom: 10px;"
                                )
                                ...
                                with (
                                    ...
                                    .classes("w-full")
                                    .style(
                                        f"height: 6px; background-color: {BG_PAGE}; border-radius: 3px; overflow: hidden; margin-bottom: 10px;"
                                    )
                                ):
                                    ui.element("div").style(
                                        f"width: {agent['progress']}%; height: 100%; background-color: {status_color}; border-radius: 3px;"
                                    )
                ...
                with (
                    ...
                    ui.label("📋 实时日志").classes("font-bold").style(
                        f"color: {TEXT_PRIMARY}; font-size: 15px; margin-bottom: 12px;"
                    )
                    log_container = (
                        ui.column()
                    ...
                    for log in logs:
                        ui.label(log).style(
                            f"color: {TEXT_SECONDARY}; font-size: 13px; font-family: monospace;"
                        )

            # === 右侧: 25% CEO面板 ===
            with ui.column().style(
                f"width: 25%; height: 100%; overflow-y: auto; padding: 20px 24px 20px 0; gap: 16px; box-sizing: border-box; border-left: 1px solid {BORDER};"
            ):
                # -- CEO Agent 信息 --
                with (
                    ui.card()
                    .classes("w-full")
                    .style(
                        f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 10px; padding: 16px;"
                    )
                ):
                    with (
                        ui.row()
                    ...
                    ui.label(
                        "执行计划已启动。Tech Researcher 正在调研制造业案例，预计 8 分钟完成。如有调整请告知。"
                    ).style(
                        f"color: {TEXT_SECONDARY}; font-size: 13px; line-height: 1.6;"
                    )
                ...
                with (
                    ...
                    ui.label("当前执行计划").classes("font-bold").style(
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 10px; padding: 16px; margin-top: auto;"
                    )
                ):
...
print("[Exp-1 v3] 请在浏览器打开: http://localhost:8501")
print("[Exp-1 v3] 按 Ctrl+C 停止服务器")
