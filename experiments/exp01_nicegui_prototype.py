...
# ========== 模拟数据 ==========
agent_status = {
    "agent_main": {"status": "running", "progress": 80, "cost": 12.50, "action": "审查代码...", "icon": "🤖"},
    "agent_audit": {"status": "idle", "progress": 0, "cost": 3.20, "action": "等待任务", "icon": "🔍"},
    "agent_dev": {"status": "waiting", "progress": 0, "cost": 8.30, "action": "等待审查结果", "icon": "💻"},
}

status_colors = {
    "idle": "#6c757d", "running": "#28a745", 
    "waiting": "#ffc107", "error": "#dc3545", "paused": "#17a2b8"
}
status_texts = {
    "idle": "空闲", "running": "运行中", 
    ...
    with ui.column().classes("w-full h-screen bg-gray-900 text-white p-4 gap-4"):
        ...
        with ui.row().classes("w-full gap-4"):
            ...
            for title, value, color in kpi_data:
                with ui.card().classes("bg-gray-800 p-4 flex-1 rounded-lg border border-gray-700"):
                    ui.label(title).classes("text-gray-400 text-sm")
                    ui.label(value).classes("text-2xl font-bold mt-2").style(f"color: {color};")
        ...
        with ui.row().classes("w-full flex-grow gap-4"):
            ...
            with ui.column().classes("w-3/4 gap-4"):
                    ...
                    for agent_id, data in agent_status.items():
                        with ui.card().classes("bg-gray-800 p-4 flex-1 rounded-lg border border-gray-700"):
                            with ui.row().classes("items-center gap-2"):
                                ...
                                ui.element("div").classes("w-3 h-3 rounded-full").style(f"background-color: {status_colors[data['status']]};")
                            ui.label(status_texts[data['status']]).classes("text-gray-400 text-sm mt-2")
                            with ui.row().classes("items-center gap-2 mt-2"):
                                ui.label("进度:").classes("text-xs text-gray-500")
                                ui.linear_progress(value=data["progress"]/100).classes("flex-grow")
                                ui.label(f"{data['progress']}%").classes("text-xs text-gray-500 w-10 text-right")
                            ui.label(f"成本: ¥{data['cost']:.2f} | {data['action']}").classes("text-gray-500 text-xs mt-2")
                
                # 日志面板
                with ui.card().classes("bg-gray-900 p-4 rounded-lg border border-gray-700"):
                ...
                # 任务结果区
                with ui.card().classes("bg-gray-800 p-4 rounded-lg border border-gray-700 min-h-32"):
                    ...
                    with ui.column().classes("items-center justify-center w-full h-32"):
                        ...
                        ui.label("在右侧CEO面板输入任务主题").classes("text-gray-600 text-xs")
            
            # 右侧: CEO面板
            with ui.column().classes("w-1/4"):
                with ui.card().classes("bg-gray-800 p-4 rounded-lg border border-gray-700 h-full"):
                    ...
                    def on_send():
                        ...
                        logs.append(f"{time.strftime('%H:%M')} [用户] {input_field.value}")
                    ...
                    for label, cmd in shortcuts:
                        ui.button(label, on_click=lambda c=cmd: ui.notify(f"发送: {c}")).classes(
...
print(f"[Exp-1] 请在浏览器打开: http://localhost:8501")
print(f"[Exp-1] 按 Ctrl+C 停止服务器")
