...
def call_ollama(prompt: str, timeout: int = 60) -> dict:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"success": False, "error": f"HTTP {resp.status_code}: {resp.text}", "latency_ms": latency}
    except requests.exceptions.ConnectionError:
        return {"success": False, "error": f"无法连接到Ollama ({OLLAMA_HOST})", "latency_ms": 0}
...
def evaluate_quality(task_type: str, output: str, criteria: str) -> int:
    ...
    if task_type == "intent_parse":
        ...
        if len(output) < 60 and ("平台" in output or "Agent" in output or "创始人" in output):
            score = 4
    elif task_type == "qa":
        if any(w in output for w in ["并发", "WAL", "恢复", "性能", "读写"]):
            score = 4
    elif task_type == "creative":
        if 8 <= len(output) <= 20 and ("伴" in output or "长" in output or "心" in output):
            score = 4
    elif task_type == "data_analysis":
        if "4月" in output or "310" in output or "超支" in output or "上升" in output:
            score = 4
    elif task_type == "chat":
        if len(output) < 120 and ("Agent" in output or "任务" in output or "成本" in output or "开发" in output):
            score = 4
