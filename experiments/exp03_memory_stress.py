...
def run_stress_test():
    ...
    if baseline.process_mb > 0:
        ...
        print(f"  系统已用: {baseline.system_used_mb:.1f}MB / {baseline.system_total_mb:.1f}MB")
    ...
    if monitor.peak_process_mb > 0:
        ...
        if final.ollama_mb == 0 and not ollama_running:
            ...
            print(f"  Ollama估算: +1800MB (qwen2.5:3b)")
    ...
    with open("experiments/results/exp03_results.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "estimated_total_mb": estimated_total if 'estimated_total' in dir() else 0,
