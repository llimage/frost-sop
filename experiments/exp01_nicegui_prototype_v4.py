...
try:
    with ui.element('div').style(f'background-color: {BG_PAGE}; display: flex; flex-direction: column; height: 100vh; overflow: hidden;'):
        ...
        with ui.row().classes('w-full').style('flex-grow: 1; min-height: 0; overflow: hidden;'):
            ...
            # === 左侧: 75% 工作区 ===
            with ui.column().style(f'width: 75%; height: 100%; overflow-y: auto; padding: 16px 20px; gap: 16px; box-sizing: border-box;'):
                ...
                with ui.row().classes('w-full gap-3').style('flex-shrink: 0;'):
                    ...
                    for big, small, label in kpi_items:
                        with ui.card().classes('flex-1').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 16px; min-height: 80px; display: flex; flex-direction: column; justify-content: center;'):
                            with ui.row().classes('items-baseline gap-1'):
                                ui.label(big).classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 28px;')
                                ui.label(small).style(f'color: {TEXT_SECONDARY}; font-size: 14px;')
                            ui.label(label).style(f'color: {TEXT_MUTED}; font-size: 12px; margin-top: 4px;')
                
                # -- 预算消耗条 --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 12px 16px; flex-shrink: 0;'):
                    with ui.row().classes('w-full items-center justify-between').style('margin-bottom: 8px;'):
                        ui.label("预算消耗").style(f'color: {TEXT_SECONDARY}; font-size: 13px; font-weight: 500;')
                        with ui.row().classes('items-center gap-3'):
                    ...
                    with ui.element('div').classes('w-full').style(f'height: 6px; background-color: {BG_PAGE}; border-radius: 3px; overflow: hidden;'):
                ...
                with ui.row().classes('w-full gap-3').style('flex-shrink: 0;'):
                    for agent in agents:
                        ...
                        with ui.card().classes('flex-1').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px; position: relative; overflow: hidden;'):
                            ...
                            with ui.row().classes('items-center gap-2').style('margin-bottom: 8px;'):
                                ...
                                with ui.column().style('gap: 0px;'):
                                    ui.label(agent["name"]).classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 14px;')
                            ...
                            ui.label(status_texts[agent["status"]]).style(f'color: {status_color}; font-size: 12px; font-weight: 500; margin-bottom: 8px;')
                            with ui.element('div').classes('w-full').style(f'height: 4px; background-color: {BG_PAGE}; border-radius: 2px; overflow: hidden; margin-bottom: 8px;'):
                ...
                # -- 实时日志 (左侧整列，高度更大) --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px 16px; flex-shrink: 0;'):
                    ui.label("📋 实时日志").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 14px; margin-bottom: 10px;')
                    log_container = ui.column().classes('w-full gap-2').style('max-height: 200px; overflow-y: auto;')
                    for log in logs:
                        ui.label(log).style(f'color: {TEXT_SECONDARY}; font-size: 12px; font-family: monospace; line-height: 1.5;')
                
                # -- Agent协同讨论 (原日志位置) --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px 16px; flex-shrink: 0;'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom: 10px;'):
                        ui.label("🤝").style('font-size: 16px;')
                        ui.label("Agent 协同讨论").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 14px;')
                    comm_container = ui.column().classes('w-full gap-3').style('max-height: 180px; overflow-y: auto;')
                    for msg in agent_comm:
                        highlight = msg.get("highlight", False)
                        bg = "rgba(59, 130, 246, 0.1)" if highlight else "transparent"
                        border_left = f"3px solid {ACCENT_BLUE}" if highlight else f"1px solid {BORDER}"
                        with ui.row().classes('w-full items-start gap-2').style(f'padding: 8px 10px; border-radius: 6px; background-color: {bg}; border-left: {border_left};'):
                            ui.label(f"{msg['time']}").style(f'color: {TEXT_MUTED}; font-size: 11px; width: 40px; flex-shrink: 0;')
                            with ui.column().style('gap: 2px;'):
                                ui.label(f"[{msg['from']}] → [{msg['to']}]").style(f'color: {TEXT_SECONDARY}; font-size: 11px; font-weight: 500;')
                                ui.label(msg["content"]).style(f'color: {TEXT_PRIMARY if highlight else TEXT_SECONDARY}; font-size: 12px; line-height: 1.4;')
            
            # === 右侧: 25% CEO面板 ===
            with ui.column().style(f'width: 25%; height: 100%; overflow-y: auto; padding: 16px 20px 16px 0; gap: 14px; box-sizing: border-box; border-left: 1px solid {BORDER};'):
                
                # -- CEO Agent 信息 --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px;'):
                    ...
                    ui.label("执行计划已启动。开发Agent正在实现情绪日记模块，预计 15 分钟完成。如有调整请告知。").style(f'color: {TEXT_SECONDARY}; font-size: 12px; line-height: 1.5;')
                
                # -- 执行计划步骤 --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px;'):
                    ui.label("执行计划").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 13px; margin-bottom: 10px;')
                    with ui.column().classes('w-full gap-2'):
                        for step, done in [("1. 需求讨论", True), ("2. 设计评审", True), ("3. 代码实现", True), ("4. 测试验证", False), ("5. 审计封板", False)]:
                            ...
                            icon = "✓" if done else "○"
                            ui.label(f"{icon} {step}").style(f'color: {color}; font-size: 12px;')
                
                # -- 对话输入区 --
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px;'):
                    ui.label("和 CEO Agent 对话").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 13px; margin-bottom: 10px;')
                    ...
                    with ui.row().classes('w-full items-center justify-end').style('margin-top: 8px;'):
                ...
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px;'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom: 10px;'):
                        ui.label("📅").style('font-size: 16px;')
                        ui.label("日程提醒").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 13px;')
                    with ui.column().classes('w-full gap-3'):
                        for item in schedule_items:
                            type_color = {"project": ACCENT_BLUE, "review": STATUS_WAIT, "cost": STATUS_RUN}
                            color = type_color.get(item["type"], TEXT_MUTED)
                            with ui.row().classes('items-start gap-2'):
                                ui.element('div').classes('w-1 h-1 rounded-full').style(f'background-color: {color}; margin-top: 6px; flex-shrink: 0;')
                                with ui.column().style('gap: 1px;'):
                                    ui.label(item["time"]).style(f'color: {TEXT_MUTED}; font-size: 11px;')
                                    ui.label(item["title"]).style(f'color: {TEXT_SECONDARY}; font-size: 12px;')
...
print(f"[Exp-1 v4] 请在浏览器打开: http://localhost:8501")
print(f"[Exp-1 v4] 按 Ctrl+C 停止服务器")
