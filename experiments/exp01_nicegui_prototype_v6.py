...
try:
    with ui.element('div').style(f'background-color: {BG_PAGE}; display: flex; flex-direction: column; height: 100vh; overflow: hidden;'):
        ...
        # ===== 主体: 三栏布局 =====
        with ui.element('div').classes('w-full').style('flex-grow: 1; min-height: 0; overflow: hidden; display: flex; flex-direction: row;'):
            
            # === 左栏 25%: 实时日志 ===
            with ui.element('div').style(f'flex: 0 0 25%; height: 100%; overflow: hidden; box-sizing: border-box; border-right: 1px solid {BORDER}; display: flex; flex-direction: column; background-color: {BG_PAGE};'):
                # 头部
                with ui.row().classes('w-full items-center gap-2').style(f'padding: 14px 16px; flex-shrink: 0; border-bottom: 1px solid {BORDER};'):
                    ui.label("📋").style('font-size: 16px;')
                    ui.label("实时日志").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 14px;')
                
                # 日志内容区 (可滚动，占满剩余高度)
                with ui.element('div').classes('w-full').style(f'flex-grow: 1; overflow-y: auto; padding: 12px 14px; min-height: 0;'):
                    log_container = ui.column().classes('w-full').style('gap: 8px;')
                    with log_container:
                        for log in logs:
                            with ui.column().classes('w-full').style(f'padding: 6px 8px; border-radius: 4px; background-color: {BG_CARD};'):
                                ui.label(log).style(f'color: {TEXT_SECONDARY}; font-size: 11px; font-family: monospace; line-height: 1.5;')
            
            # === 中栏 50%: KPI + 预算 + Agent + 协同 ===
            with ui.element('div').style(f'flex: 0 0 50%; height: 100%; overflow-y: auto; padding: 14px 16px; gap: 12px; box-sizing: border-box; display: flex; flex-direction: column; background-color: {BG_PAGE};'):
                ...
                with ui.row().classes('w-full gap-3').style('flex-shrink: 0;'):
                    ...
                    for big, small, label in kpi_items:
                        with ui.card().classes('flex-1').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 12px; min-height: 65px; display: flex; flex-direction: column; justify-content: center;'):
                            with ui.row().classes('items-baseline gap-1'):
                                ui.label(big).classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 24px;')
                                ui.label(small).style(f'color: {TEXT_SECONDARY}; font-size: 12px;')
                ...
                with ui.card().classes('w-full').style(f'background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 12px 14px; flex-grow: 1; display: flex; flex-direction: column; min-height: 0;'):
                    with ui.row().classes('items-center gap-2').style('margin-bottom: 8px; flex-shrink: 0;'):
                        ui.label("🤝").style('font-size: 14px;')
                        ui.label("Agent 协同讨论").classes('font-bold').style(f'color: {TEXT_PRIMARY}; font-size: 13px;')
                    comm_container = ui.element('div').classes('w-full').style('flex-grow: 1; overflow-y: auto; min-height: 0;')
                    with comm_container:
            ...
            # === 右栏 25%: CEO Agent聊天面板 ===
            with ui.element('div').style(f'flex: 0 0 25%; height: 100%; overflow: hidden; box-sizing: border-box; border-left: 1px solid {BORDER}; display: flex; flex-direction: column; background-color: {BG_PAGE};'):
                ...
                # -- 输入区 (固定在底部) --
                with ui.element('div').classes('w-full').style(f'padding: 10px 14px; flex-shrink: 0; border-top: 1px solid {BORDER}; background-color: {BG_PAGE};'):
                    
                    # 输入框 + 发送按钮 同一行
                    with ui.row().classes('w-full items-start gap-2').style('margin-bottom: 8px;'):
                        input_field = ui.textarea(placeholder="和 CEO Agent 对话...").classes('flex-grow').style(
...
print(f"[Exp-1 v6] 请在浏览器打开: http://localhost:8501")
print(f"[Exp-1 v6] 按 Ctrl+C 停止服务器")
