    ...
    with ui.element("div").style(
        ...
        with (
            ...
            with ui.column().style(
                "width: 75%; height: 100%; overflow-y: auto; padding: 16px 20px; gap: 14px; box-sizing: border-box;"
            ):
                # -- KPI卡片 (4个等大小，紧凑) --
                with ui.row().classes("w-full gap-3").style("flex-shrink: 0;"):
                    ...
                    for big, small, label in kpi_items:
                        with (
                            ...
                            .classes("flex-1")
                            .style(
                                f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 14px; min-height: 70px; display: flex; flex-direction: column; justify-content: center;"
                            )
                        ):
                            with ui.row().classes("items-baseline gap-1"):
                                ui.label(big).classes("font-bold").style(
                                    f"color: {TEXT_PRIMARY}; font-size: 26px;"
                                )
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        f"background-color: {BG_CARD}; border: 1px solid {BORDER}; border-radius: 8px; padding: 12px 14px; flex-shrink: 0;"
                    )
                ):
                    ui.label("📋 实时日志").classes("font-bold").style(
                        f"color: {TEXT_PRIMARY}; font-size: 13px; margin-bottom: 8px;"
                    )
                    log_container = (
                        ui.column()
            ...
            # === 右侧: 25% 聊天面板 ===
            with ui.column().style(
                f"width: 25%; height: 100%; overflow: hidden; box-sizing: border-box; border-left: 1px solid {BORDER};"
            ):
                # -- CEO Agent 头部信息 --
                with (
                    ui.row()
                ...
                chat_scroll = (
                    ...
                    .classes("w-full")
                    .style(
                        "flex-grow: 1; overflow-y: auto; padding: 12px 14px; gap: 10px;"
                    )
                )
                ...
                with (
                    ...
                    .classes("w-full")
                    .style(
                        f"padding: 10px 14px; flex-shrink: 0; border-top: 1px solid {BORDER}; gap: 8px;"
                    )
                ):
                    input_field = (
...
print("[Exp-1 v5] 请在浏览器打开: http://localhost:8501")
print("[Exp-1 v5] 按 Ctrl+C 停止服务器")
