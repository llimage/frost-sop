...
def _make_output_skill_func(name, desc, reason_prefix, output_type="document"):
    ...
    def skill_func(ctx):
        ...
        ctx["_result"] = result_ctx.get(
            "_generated_content", f"[{name}] {desc}：任务执行完成"
        )
...
""".format(requirement, json.dumps(template_list, ensure_ascii=False, indent=2))
    ...
    context["_reason"] = (
        f"Agent组装完成: {child.name}，{len(assembled_skills)}个Skill（基因库:{len(assembled_skills) - len(missing_skills)} 合成:{len(missing_skills)}）"
    )
