...
def execute_stage(context: dict) -> dict:
    ...
    requires_decision = any(
        keyword in stage_name.lower() for keyword in decision_keywords
    )
    ...
    if requires_decision:
        ...
        if task_id == "unknown":
            ...
            decision_id = decision_manager.pause_decision(
                task_id=task_id, stage_id=stage_id, question=question, options=options
            )
    ...
    context["_reason"] = (
        f"孙辈Agent(组装)执行阶段'{stage_name}'完成。Skills: {agent_config.get('skills')}（来源: {agent_config.get('skill_sources')}）"
    )
