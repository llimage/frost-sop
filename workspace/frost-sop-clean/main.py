        ...
        bus.publish(
            Event(
                ...
                data={
                    ...
                    "stages": [
                        s.get("name", f"阶段{i + 1}")
                        for i, s in enumerate(sop_stages_detail)
                    ],
                },
            )
        )
        logger.info(
            "  📡 [V2.0] TASK_DECOMPOSED 事件已发布（%s个阶段）", len(sop_stages_detail)
        )
...
async def main_async(task_input: str = None, sop_id: str = None, timeout: int = 600):
    ...
    if final_status["status"] == "completed":
        ...
        event = final_status["event"]
        logger.error(
            "[V3.0] 任务失败: %s — %s", task_id, event.data.get("error", "unknown")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sop", type=str, default=None, help="SOP template ID (e.g. DEV-001)"
    )
    parser.add_argument(
        "--async-mode", action="store_true", help="Use V3.0 async event-driven mode"
    )
    parser.add_argument(
        "--timeout", type=int, default=600, help="Timeout in seconds (async mode)"
    )
