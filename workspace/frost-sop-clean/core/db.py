...
class DBManager:
    ...
    def _migrate_energy_log_table(self, cursor):
        ...
        for col_name, col_def in needed.items():
            if col_name not in existing:
                try:
                    cursor.execute(
                        f"ALTER TABLE energy_log ADD COLUMN {col_name} {col_def}"
                    )
                except Exception:
                    pass  # 列可能已存在
    ...
    def _migrate_schedule_table(self, cursor):
        ...
        existing = {
            col["name"]
            for col in cursor.execute("PRAGMA table_info(schedule)").fetchall()
        }
        for col_name, col_def in needed.items():
            if col_name not in existing:
                try:
                    cursor.execute(
                        f"ALTER TABLE schedule ADD COLUMN {col_name} {col_def}"
                    )
                except Exception:
                    pass
    ...
    def _migrate_skill_versions_table(self, cursor):
        ...
        if "file_path" not in existing:
            try:
                cursor.execute(
                    "ALTER TABLE skill_versions ADD COLUMN file_path TEXT DEFAULT ''"
                )
            except Exception:
                pass
    ...
    def _migrate_decision_points_table(self, cursor):
        ...
        existing = {
            col["name"]
            for col in cursor.execute("PRAGMA table_info(decision_points)").fetchall()
        }
        for col_name, col_def in needed.items():
            if col_name not in existing:
                try:
                    cursor.execute(
                        f"ALTER TABLE decision_points ADD COLUMN {col_name} {col_def}"
                    )
                except Exception:
                    pass
    ...
    def select_all(
        self, table: str, where: str = None, params: List[Any] = None
    ) -> List[Dict[str, Any]]:
        """
        查询多条记录
