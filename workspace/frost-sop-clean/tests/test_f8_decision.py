...
class TestDecisionManager:
    ...
    def test_resume_decision(self):
        ...
        result = self.decision_manager.resume_decision(
            decision_id=decision_id, user_choice="confirm", user_note="测试备注"
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT status, user_decision, user_note FROM decision_points WHERE id = ?",
            (decision_id,),
        )
        row = cursor.fetchone()
        # 不关闭连接 — DB 是单例模式
    ...
    def test_has_pending_decision(self):
        """测试检查是否有 pending 决策"""
        # 初始状态：没有 pending 决策
        assert not self.decision_manager.has_pending_decision(), (
            "初始状态应无 pending 决策"
        )
...
class TestNotifier:
    ...
    def test_check_decision_timeout(self):
        ...
        assert not check_decision_timeout(recent_time, timeout_seconds=3600), (
            "30分钟前不应超时"
        )

        # 测试3：字符串时间格式
        old_time_str = (datetime.now() - timedelta(hours=2)).isoformat()
        assert check_decision_timeout(old_time_str, timeout_seconds=3600), (
            "字符串时间格式应正常工作"
        )
    ...
    def test_send_notification(self):
        ...
        result = send_windows_notification(
            title="F8 测试通知", message="这是一条测试通知", duration=1
        )
...
class TestAuditIntegration:
    ...
    def teardown_method(self):
        ...
        if hasattr(self, "db"):
            ...
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM decision_points WHERE task_id = ?", (self.task_id,)
            )
    ...
    def test_audit_log_after_decision(self):
        ...
        self.decision_manager.resume_decision(
            decision_id=decision_id, user_choice="confirm", user_note="审计测试备注"
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT action, agent_id FROM audit_log WHERE id = ?", (audit_id,)
        )
        row = cursor.fetchone()
