...
class TestF9Schedule:
    ...
    def setup_class(cls):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM schedule WHERE name LIKE 'F9测试%' OR title LIKE 'F9测试%'"
        )
        conn.commit()
    ...
    def test_get_schedules(self):
        ...
        assert len(test_schedules) >= 1, (
            f"应有至少1条测试日程，得到: {len(test_schedules)}"
        )
    ...
    def test_delete_schedule(self):
        ...
        sid = self.db.add_schedule(
            title="F9测试删除日程", start_time=tomorrow, end_time=end_time
        )
    ...
    def test_get_upcoming_reminders(self):
        ...
        sid = self.db.add_schedule(
            title="F9测试提醒日程", start_time=start, end_time=end
        )
        ...
        if test_reminders:
            ...
            test_reminders2 = [
                u for u in upcoming2 if u["id"] == test_reminders[0]["id"]
            ]
