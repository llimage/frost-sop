...
def test_cost_tracker_budget_check():
    ...
    try:
        ...
        assert budget_info["status"] in ["healthy", "warning"], (
            "预算状态应该是healthy或warning"
        )
...
if __name__ == "__main__":
    ...
    print("\n子任务1：SQLite持久化")
    results.append(
        test_sqlite_persistence_task(temp_db=tempfile.mkdtemp() + "/test.db")
    )
    results.append(
        test_sqlite_persistence_skill_gene(temp_db=tempfile.mkdtemp() + "/test.db")
    )
