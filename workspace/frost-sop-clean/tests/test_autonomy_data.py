...
def verify_audit_report(
    report: Dict[str, Any], expected_num_tasks: int
) -> Dict[str, Any]:
    """
    验证审计报告的数据真实性
        ...
        checks.append(
            {
                "name": "findings非空",
                "passed": True,
                "detail": f"报告包含{len(findings)}个发现",
            }
        )
    else:
        checks.append(
            {"name": "findings非空", "passed": False, "detail": "报告不包含任何发现"}
        )
...
def verify_succession_proposal(
    proposal: Dict[str, Any], expected_num_tasks: int
) -> Dict[str, Any]:
    """
    验证交棒评估的数据真实性
...
def test_autonomy_data():
    ...
    if len(task_keys) != 5:
        ...
        return {
            "passed": False,
            "reason": f"预置数据失败: 期望5条, 实际{len(task_keys)}条",
        }
    ...
    try:
        ...
        if audit_skill is None:
            ...
            audit_result = {
                "passed": False,
                "reason": "长老Agent缺少audit_family Skill",
            }
        else:
    ...
    all_passed = audit_result.get("passed", False) and succession_result.get(
        "passed", False
    )
    ...
    print(
        f"交棒评估: {'✅ 通过' if succession_result.get('passed', False) else '❌ 失败'}"
    )
