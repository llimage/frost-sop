...
from skills.intent import parse_intent, list_all_sops
...
class TestIntentParsing:
    ...
    def test_parse_dev_task(self):
        ...
        assert result["sop_id"] == "DEV-001", (
            f"应匹配 DEV-001, 实际: {result['sop_id']}"
        )
        assert result["confidence"] > 0.5, (
            f"置信度应 > 0.5, 实际: {result['confidence']}"
        )
        print(
            f"  ✅ test_parse_dev_task: sop={result['sop_id']} conf={result['confidence']}"
        )
    ...
    def test_parse_bug_fix(self):
        ...
        assert result["sop_id"] == "DEV-002", (
            f"应匹配 DEV-002, 实际: {result['sop_id']}"
        )
    ...
    def test_parse_finance(self):
        ...
        assert result["sop_id"] == "OPS-001", (
            f"应匹配 OPS-001, 实际: {result['sop_id']}"
        )
    ...
    def test_parse_knowledge(self):
        ...
        assert result["sop_id"] == "OPS-006", (
            f"应匹配 OPS-006, 实际: {result['sop_id']}"
        )
    ...
    def test_parse_project_init(self):
        ...
        assert result["sop_id"] == "STR-001", (
            f"应匹配 STR-001, 实际: {result['sop_id']}"
        )
    ...
    def test_parse_unknown_type(self):
        ...
        assert result["sop_id"] is None, (
            f"无对应SOP时应为 None, 实际: {result['sop_id']}"
        )
        assert result["confidence"] < 0.5, (
            f"低置信度应 < 0.5, 实际: {result['confidence']}"
        )
        print(
            f"  ✅ test_parse_unknown_type: sop={result['sop_id']} conf={result['confidence']}"
        )
    ...
    def test_llm_mode_fallback(self):
        ...
        print(
            f"  ✅ test_llm_mode_fallback: method={result['method']} sop={result['sop_id']}"
        )
