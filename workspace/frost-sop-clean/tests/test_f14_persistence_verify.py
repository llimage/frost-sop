...
def verify_persistence(task_id, sop_exec_id):
    ...
    cursor.execute(
        "SELECT COUNT(*) as cnt FROM task_stages WHERE task_id = ?", [task_id]
    )
...
def verify_cost_log_association(task_id):
    ...
    # Get sample records
    cursor.execute(
        "SELECT agent_id, task_id, total_tokens FROM cost_log ORDER BY id DESC LIMIT 5"
    )
...
def main():
    ...
    for s in cost_check["samples"]:
        print(
            f"     agent={s['agent_id']}, task={s['task_id']}, tokens={s['total_tokens']}"
        )
