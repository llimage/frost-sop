...
from core.panel import PanelComponent, PanelDefinition, PanelState
...
class CliRenderer(PanelRenderer):
    ...
    def render(self, panel: PanelDefinition, state: PanelState | None = None) -> None:
        """渲染面板到终端"""
        # 1. 渲染标题
    ...
    def _render_decision_buttons(self, component: PanelComponent, data: Any) -> None:
        ...
        self.console.print(f"[yellow]输入选项编号（1-{len(options)}）:[/yellow] ", end="")
    ...
    def _render_alert_banner(self, component: PanelComponent, data: Any) -> None:
        ...
        if isinstance(data, list):
            for alert in data:
                ...
                # 使用 [!] 代替 ⚠️
                marker = {"critical": "[XXX]", "high": "[!!]", "medium": "[!]", "low": "[i]"}.get(
                    level, "[?]"
                )
