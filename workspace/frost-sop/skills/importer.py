...
def import_agency_agents(context: dict) -> dict:
    ...
    for md_file in agency_dir.rglob("*.md"):
        # 跳过非角色定义文件
        if md_file.name.startswith(".") or md_file.name.lower() in [
            "readme.md",
            "catalog.md",
            "agency-list.md",
        ]:
        ...
        try:
            ...
            if not gene_data.get("category") or gene_data["category"] == "imported":
                ...
                if any(
                    kw in name_lower for kw in ["开发", "engineer", "dev", "前端", "后端", "全栈"]
                ):
                    gene_data["category"] = "engineering"
                elif any(
                    kw in name_lower for kw in ["营销", "market", "运营", "文案", "小红书", "抖音"]
                ):
