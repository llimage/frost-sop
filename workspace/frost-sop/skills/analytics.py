...
def analyze_finance(context: dict) -> dict:
    ...
    # 统计
    total_cost = sum(item.get("value", 0) for item in cost_data if item.get("metric_type") == "llm_cost")
    total_tokens = sum(item.get("value", 0) for item in cost_data if item.get("metric_type") == "llm_token_usage")
    
    # 预算使用率
    budget_usage_rates = [item for item in cost_data if item.get("metric_type") == "budget_usage_rate"]
    ...
    # 根据深度决定是否调用LLM
    if depth == "light":
        result["briefing"] = f"财务简报（轻量）：本月成本 ${total_cost:.4f}，Token消耗 {total_tokens:.0f}，预算使用率 {latest_budget_rate:.1%}"
...
def analyze_skill(context: dict) -> dict:
    ...
    # 根据深度决定是否调用LLM
    if depth == "light":
        result["briefing"] = f"Skill简报（轻量）：总执行 {total_executions} 次，成功率 {success_rate:.1%}"
...
def analyze_task(context: dict) -> dict:
    ...
    # 根据深度决定是否调用LLM
    if depth == "light":
        result["briefing"] = f"任务简报（轻量）：总任务 {total_tasks} 个，完成率 {completion_rate:.1%}，平均耗时 {avg_duration:.1f}s"
...
def analyze_heartbeat(context: dict) -> dict:
    ...
    # 根据深度决定是否调用LLM
    if depth == "light":
        result["briefing"] = f"心跳简报（轻量）：总心跳 {total_heartbeats} 次，超时率 {timeout_rate:.1%}"
...
def analyze_hunt(context: dict) -> dict:
    ...
    # 根据深度决定是否调用LLM
    if depth == "light":
        result["briefing"] = f"狩猎简报（轻量）：总狩猎 {total_hunts} 次，吸收率 {absorption_rate:.1%}"
