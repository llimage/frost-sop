...
def _write_collected_data(
    store, source: str, metric_type: str, value: float, tags: dict[str, Any]
) -> bool:
    """
    辅助函数：将标准化数据写入Store。
...
def collect_task_data(context: dict) -> dict:
    ...
    tags = {"task_id": task_id, "status": task_status, "duration_seconds": duration}
...
def collect_cost_data(context: dict) -> dict:
    ...
    **P0-a中第一个实现。**
    ...
    success_token = _write_collected_data(
        store, "collector_cost", "llm_token_usage", input_tokens + output_tokens, tags_token
    )

    # 写入成本指标
    tags_cost = {"task_id": task_id, "model": model, "cost_usd": cost}
    ...
    success_budget = _write_collected_data(
        store, "collector_cost", "budget_usage_rate", budget_usage_rate, tags_budget
    )
...
def collect_heartbeat_data(context: dict) -> dict:
    ...
    tags = {"agent_id": agent_id, "status": heartbeat_status, "idle_seconds": idle_seconds}
...
def collect_hunt_data(context: dict) -> dict:
    ...
    tags = {"task_id": task_id, "skill_id": skill_id, "result": hunt_result, "score": hunt_score}
    if hunt_reason:
