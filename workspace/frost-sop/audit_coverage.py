...
def audit_coverage():
    ...
    # 解析覆盖率结果
    try:
        with open("audit_results/coverage.json", encoding="utf-8") as f:
...
def generate_coverage_report():
    ...
    # 读取未覆盖代码
    try:
        with open("audit_results/uncovered_lines.json", encoding="utf-8") as f:
    ...
    for module, coverage in report["module_coverage"].items():
        if coverage < 70:
            report["recommendations"].append(
                f"模块 {module}/ 覆盖率较低 ({coverage}%)，建议重点补充测试"
            )
    ...
    print("[AUDIT] ✅ 覆盖率审计报告已保存: audit_results/coverage_audit_report.json")
