...
def collect_audit_results():
    ...
    # 1. 代码质量结果
    try:
        with open("audit_results/code_quality_summary.json", encoding="utf-8") as f:
    ...
    # 2. 安全审计结果
    try:
        with open("audit_results/security_audit_report.json", encoding="utf-8") as f:
    ...
    # 3. 覆盖率审计结果
    try:
        with open("audit_results/coverage_audit_report.json", encoding="utf-8") as f:
...
def generate_final_report(results):
    ...
    if results["summary"]["total_issues"] == 0:
        ...
        report["conclusions"].append(
            f"⚠️  发现 {results['summary']['total_issues']} 个问题，建议修复后重新审计"
        )
