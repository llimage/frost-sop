...
from skills.assemble import (
    _collect_templates,
    _create_skills_from_genes,
    _keyword_fallback,
    assemble_agent,
    create_skill_from_gene,
    synthesize_skill,
)
...
def test_collect_templates_with_genes():
    """有skill_gene数据时正确收集"""
    store = Store()
    store.save("skill_gene:test_skill", {"name": "test_skill", "description": "测试技能"})
...
def test_keyword_fallback_match():
    """关键词匹配命中"""
    store = Store()
    store.save("skill_gene:需求分析", {"name": "需求分析", "description": "分析需求"})
    store.save("skill_gene:代码生成", {"name": "代码生成", "description": "写代码"})
...
def test_synthesize_skill_with_store():
    """合成Skill并归档到Store"""
    store = Store()
    skill = synthesize_skill("归档技能", "归档测试", asset_store=store, output_type="code")
    assert isinstance(skill, Skill)
    # 验证归档
