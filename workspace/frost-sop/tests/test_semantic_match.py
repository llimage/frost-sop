    ...
    for test_case in TEST_CASES:
        ...
        try:
            ...
            if assembled_agent is None:
                print(f"  ❌ 匹配失败: {reason}")
                results.append(
                    {"id": test_case["id"], "passed": False, "reason": f"匹配失败: {reason}"}
                )
                continue
            ...
            results.append({"id": test_case["id"], "passed": False, "reason": f"执行异常: {e}"})
