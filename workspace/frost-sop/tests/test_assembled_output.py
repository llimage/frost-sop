...
TEST_CASES = [
    {
        ...
        "quality_checks": {
            "contains_tags": ["<form", "<input", "<button"],
            "min_length": 50
        }
    },
    {
...
def check_quality(output: str, checks: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "min_length" in checks:
        if len(output) < checks["min_length"]:
            failed_checks.append(f"长度不足: 期望至少{checks['min_length']}字符, 实际{len(output)}字符")
...
def test_assembled_output():
    ...
    for test_case in TEST_CASES:
        ...
        try:
            ...
            # 准备任务context
            task_context = {
                "_task_description": test_case["task"],
                "_output_type": "text"
            }
