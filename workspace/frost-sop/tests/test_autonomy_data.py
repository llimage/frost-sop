...
def verify_audit_report(report: Dict[str, Any], expected_num_tasks: int) -> Dict[str, Any]:
    ...
    if len(findings) > 0:
        checks.append({"name": "findings非空", "passed": True, "detail": f"报告包含{len(findings)}个发现"})
    else:
...
def test_autonomy_data():
    ...
    try:
        ...
        if audit_skill is None:
            print(f"  ❌ 长老Agent缺少audit_family Skill")
