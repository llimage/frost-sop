...
class TestSkillExtractor:
    ...
    def setup_class(cls):
        ...
        # 初始化数据库 - 重置单例
        from tests.conftest import reset_db_singleton

        reset_db_singleton(cls.db_path)
    ...
    def test_scan_and_extract_all(self):
        ...
        assert len(files1) >= 1, "First scan should create drafts"

        # 第二次提取（应该去重，不再新创建）
        files2 = extractor.scan_and_extract_all()
        assert len(files2) == 0, "Second scan should be deduplicated"
    ...
    def test_draft_in_database(self):
        ...
        assert len(drafts) >= 1, f"Expected at least 1 draft, got {len(drafts)}"
        for d in drafts:
    ...
    def test_validate_skill_becomes_active(self):
        ...
        row = cursor.fetchone()
        assert row is not None, "Expected at least 1 draft skill to validate"
...
class TestF10Integration:
    ...
    def test_db_migration_adds_columns(self):
        ...
        columns = {
            col["name"] for col in cursor.execute("PRAGMA table_info(skill_versions)").fetchall()
        }
