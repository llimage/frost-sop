...
def test_par01_two_dev001():
    ...
    results = []
    lock = threading.Lock()
    t1 = threading.Thread(target=_run_sop_task, args=("DEV-001", "用户认证功能", results, lock))
    t2 = threading.Thread(target=_run_sop_task, args=("DEV-001", "数据导出功能", results, lock))
    t1.start()
    t2.start()
    t1.join(timeout=60)
    t2.join(timeout=60)
...
def test_par02_mixed_types():
    ...
    lock = threading.Lock()
    t1 = threading.Thread(target=_run_sop_task, args=("DEV-001", "用户认证功能", results, lock))
    t2 = threading.Thread(target=_run_sop_task, args=("MT-001", "FROST框架推广文案", results, lock))
    t1.start()
    t2.start()
    t1.join(timeout=60)
    t2.join(timeout=60)
...
def test_par03_three_mixed():
    ...
    lock = threading.Lock()
    threads = [
        threading.Thread(target=_run_sop_task, args=("DEV-001", "用户认证", results, lock)),
        threading.Thread(target=_run_sop_task, args=("MT-001", "推广文案", results, lock)),
        threading.Thread(target=_run_sop_task, args=("OPS-001", "财务月结", results, lock)),
    ]
    for t in threads:
        t.start()
    for t in threads:
...
def _run_failing_task(sop_id: str, results: list, lock: threading.Lock):
    ...
    try:
        ...
        with lock:
            results.append({"sop_id": sop_id, "expected_failure": True, "error": str(e)})
...
def test_par04_failure_isolation():
    ...
    lock = threading.Lock()
    # 任务1：正常
    t1 = threading.Thread(target=_run_sop_task, args=("DEV-001", "正常任务", results, lock))
