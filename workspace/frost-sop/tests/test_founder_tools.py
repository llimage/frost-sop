...
class TestF9EnergyLog:
    ...
    def test_add_energy_log(self):
        """测试写入能量记录"""
        rid = self.db.add_energy_log(level=75, emotion="专注", note="测试记录")
    ...
    def test_get_latest_energy(self):
        ...
        print("  ✅ test_get_latest_energy 通过")
...
class TestF9Schedule:
    ...
    def test_get_schedules(self):
        ...
        self.db.add_schedule(
            title="F9测试-get_schedules",
            description="test_get_schedules 自建数据",
            start_time=tomorrow,
            end_time=end_time,
            repeat_type="once",
        )
        ...
        test_schedules = [
            s for s in schedules if (s.get("title") or s.get("name", "")).startswith("F9测试")
        ]
    ...
    def test_delete_schedule(self):
        ...
        sid = self.db.add_schedule(title="F9测试删除日程", start_time=tomorrow, end_time=end_time)
    ...
    def test_get_upcoming_reminders(self):
        ...
        sid = self.db.add_schedule(title="F9测试提醒日程", start_time=start, end_time=end)

        # 获取未来15分钟内的提醒
        upcoming = self.db.get_upcoming_reminders(15)
        test_reminders = [
            u for u in upcoming if (u.get("title") or u.get("name", "")).startswith("F9测试")
        ]
        ...
        if test_reminders:
            ...
            test_reminders2 = [u for u in upcoming2 if u["id"] == test_reminders[0]["id"]]
            assert len(test_reminders2) == 0, "标记后不应再出现"
            print("  ✅ test_mark_schedule_notified 通过")
        else:
...
class TestF9Notifier:
    ...
    def test_notifier_import(self):
        """测试通知模块导入"""
        from core.notifier import check_decision_timeout, send_windows_notification
...
class TestF9DBMigration:
    ...
    def test_energy_log_columns(self):
        ...
        print("  ✅ test_energy_log_columns 通过")
    ...
    def test_schedule_columns(self):
        ...
        print("  ✅ test_schedule_columns 通过")
