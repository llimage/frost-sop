...
async def test_01_main_async_publishes_task_created():
    ...
    async def capture(e): created_events.append(e)
    bus.subscribe_async(EventType.TASK_CREATED, capture)

    # 运行 main_async（mock 模式，会快速完成）
    status = await asyncio.wait_for(
        _run_main_async_safe("测试任务", "DEV-001", timeout=30),
        timeout=60
    )
...
async def test_02_main_async_returns_on_completion():
    ...
    status = await asyncio.wait_for(
        _run_main_async_safe("完成测试", "DEV-001", timeout=30),
        timeout=60
    )
...
async def test_03_main_async_supports_timeout():
    ...
    # 使用很短的超时来测试
    status = await asyncio.wait_for(
        _run_main_async_safe("超时测试", "DEV-001", timeout=1),
        timeout=30
    )
...
async def test_04_timeout_publishes_task_timeout():
    ...
    async def capture_timeout(e): timeout_events.append(e)
    async def capture_failed(e): failed_events.append(e)
    ...
    # 使用 1 秒超时
    status = await asyncio.wait_for(
        _run_main_async_safe("超时验证", "DEV-001", timeout=1),
        timeout=30
    )
