...
def run_command(cmd, output_file=None, cwd=None):
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=cwd or ".")
    ...
    if not check_tool_installed("pip-audit"):
        if not install_tool("pip-audit"):
            ...
            cmd = [
                ...
                "json",
                "--output",
                "audit_results/pip_audit_vulnerabilities.json",
            ]
            run_command(cmd)
...
def audit_static_security():
    ...
    cmd = [
        ...
        "json",
        "-o",
        "audit_results/bandit_security_issues.json",
    ...
    # 解析结果
    try:
        with open("audit_results/bandit_security_issues.json", encoding="utf-8") as f:
            data = json.load(f)
            issue_count = len(data.get("results", []))
            high_severity = len(
                [i for i in data.get("results", []) if i.get("issue_severity") == "HIGH"]
            )
...
def audit_license_compliance():
    ...
    # 检查是否有高风险协议 (GPL 等)
    try:
        with open("audit_results/license_compliance.json", encoding="utf-8") as f:
            licenses = json.load(f)
            risky_licenses = ["GPL", "AGPL", "LGPL"]
            risky_deps = [
                dep
                for dep in licenses
                if any(rl in dep.get("License", "") for rl in risky_licenses)
            ]
...
def generate_security_report():
    ...
    # 读取 Bandit 结果
    try:
        with open("audit_results/bandit_security_issues.json") as f:
            bandit_data = json.load(f)
            high_issues = [
                i for i in bandit_data.get("results", []) if i.get("issue_severity") == "HIGH"
            ]
            medium_issues = [
                i for i in bandit_data.get("results", []) if i.get("issue_severity") == "MEDIUM"
            ]
    ...
    print("[AUDIT] ✅ 安全审计报告已保存: audit_results/security_audit_report.json")
