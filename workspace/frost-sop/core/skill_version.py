...
class SkillVersionManager:
    ...
    def create_new_version(self, skill_id: str, content: str, changelog: str = "") -> int:
        """
        为 Skill 创建新版本。
        ...
        # 获取当前版本号和名称
        cursor.execute("SELECT name, version FROM skills WHERE id = ?", (skill_id,))
        row = cursor.fetchone()
        if not row:
    ...
    def rollback(self, skill_id: str, target_version: int) -> bool:
        ...
        # 创建新版本（内容为目标版本内容）
        self.create_new_version(skill_id, content, f"回滚到版本 {target_version_str}")
        return True
    ...
    def get_latest_version(self, skill_id: str) -> dict[str, Any] | None:
        """
        获取 Skill 的最新版本。
