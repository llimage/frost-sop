    ...
    def check(self, weapon_id: str, to_state: str) -> GuardResult:
        ...
        if to_state == "trialed":
            if weapon.usage_count < 1:
                warnings.append(
                    f"试炼条件建议: usage_count({weapon.usage_count}) < 1，建议先运行至少1次"
                )
...
class BatchLifecycleManager:
    ...
    def __init__(self, registry, event_log: LifecycleEventLog | None = None):
        self.registry = registry
        self.event_log = event_log or LifecycleEventLog()
        self.guard = TransitionGuard(registry)

    def batch_transition(
        self, weapon_ids: list[str], to_state: str, reason: str = "", operator: str = "system"
    ) -> BatchResult:
        """
        批量状态转换。
        ...
        for weapon_id in weapon_ids:
            ...
            if not weapon:
                skipped += 1
                details.append(
                    {"weapon_id": weapon_id, "status": "skipped", "reason": "武器不存在"}
                )
                continue
            ...
            if not guard_result.allowed:
                failed += 1
                details.append(
                    {"weapon_id": weapon_id, "status": "failed", "reason": guard_result.reason}
                )
                continue
            ...
            try:
                ...
                failed += 1
                details.append({"weapon_id": weapon_id, "status": "failed", "reason": str(e)})
    ...
    def batch_retire_low_health(
        self, threshold: float = 20.0, operator: str = "system"
    ) -> BatchResult:
        """
        批量退役低健康评分武器（跳过预置武器）。
